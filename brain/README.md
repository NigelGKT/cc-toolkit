...
`~/.claude/brain/` alongside the rest of your toolkit.
...
2. **Query (out)** — before solving something cold: *"what does my brain say about X"* →
   `s.wiki` reads `~/.claude/brain/` and answers with citations.
3. **Lint (maintain)** — periodically ask `s.wiki` to health-check for contradictions,
   stale claims, and orphans.

## The one rule

**Curate in the `cc-toolkit` repo clone — never edit the deployed `~/.claude/brain/`.**
...
Point Obsidian at the `brain/` folder (or the whole `cc-toolkit` repo). The graph view
