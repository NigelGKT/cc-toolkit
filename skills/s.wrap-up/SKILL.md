...
4. Check for a wiki, in order — stop at the first hit: (a) scan `CLAUDE.md` in CWD for a `## Wiki` section containing a path (honour any scope gate it states); (b) `./wiki-schema.md`; (c) **glob one level down for `*/wiki-schema.md`** — exactly one match wins, several → ask which. **A brain in a subfolder is the norm, not the exception — never conclude "no wiki" until (c) also comes up empty.** Skipping Stage 0 on a false negative silently loses the session note.
4b. **Split-root case:** the git repo and the wiki brain need not share a root. Where a project authors into one location and syncs to a git clone elsewhere, git orientation runs against the clone while the session note is written to the **authoring** copy. Resolve each independently rather than assuming one root serves both.
...
**C — Promote to the global toolkit (cc-toolkit)?** Anything from this session that generalizes beyond this project: a **transferable pattern/concept → `cc-toolkit-wiki-brain/concepts/`**, a **client-agnostic lesson/checklist → `cc-toolkit-wiki-brain/playbooks/`**, a reusable command → `commands/`, a skill → `skills/`, or a refined global behaviour → global `CLAUDE.md`. List each as a one-line candidate with its target, and for the two wiki-brain targets **draft the distilled page in this summary** so it's visible before the gate.

**Prose promotions (`concepts/` + `playbooks/`) are applied on the single confirmation**, not flagged: on confirm the skill distils each into the global brain (see the "On confirmation" write step) — **scrub project specifics from the body and record the source in `origin:` frontmatter** (see `cc-toolkit-wiki-brain/wiki-schema.md`). **Code/contract promotions stay flag-only** — a command, skill, or global-`CLAUDE.md` rule needs real implementation, not a distilled note, so name it as a candidate and leave it for the user to action manually. This all rides the **same single gate** — never add a second gate for it. If nothing generalizes, say "nothing to promote" and move on.
