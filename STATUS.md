...
- **Last released: v1.18.0** — `s.wiki` Bootstrap Step 0 safety gate (refuses to scaffold into a
  populated target). Run `git log` for the commit trail.
- **Active: v1.19.0 — single-pass release flow + retrieve-don't-duplicate cleanup.** A new conductor
  ...
  `CC_TOOLKIT_HOME` on deploy (the drift-check hook was silently dead without it); STATUS trimmed to
  state + intent; CHANGELOG thinned to a signpost; the retrieve-don't-duplicate rule encoded in
  `CLAUDE.md`.

## Next step

- **Verify v1.19.0 end-to-end** (see the approved plan): confirm the deploy sets `CC_TOOLKIT_HOME`
  and revives the SessionStart hook; rehearse `/s.ship-cc-tlkit` to the gate and abort (no push);
  then one supervised real run (commit stages named files only, push lands, deploy-down refreshes
  `~/.claude`). The first real ship of `s.ship-cc-tlkit` is itself the test — it ships its own session.
