...
- runtime junk (cache, history, sessions, plugins, plans) — see `.gitignore`
...
- **Existing config detected** → `setup.ps1` runs in **AUDIT MODE** and changes nothing.
  ...
  - **HARVEST CANDIDATES** — files on this machine but *not* in the repo. Pull anything
    worth keeping UP into cc-toolkit first (commit + push) so you don't lose local work.
  - **WOULD BE ADDED** — new files the toolkit would introduce.
  ...
  `~/.claude.backup-<timestamp>` first, then merge-copies the toolkit in.
